-- Keyword recall path for A′ multi-recall RAG.
-- Returns chunks ranked by title+content term hit count (keyword_rank).
-- Allows embedding-missing chunks when content is non-empty.
