...
Deno.serve(async (req) => {
  ...
  try {
    body = normalizeOutlineRequestBody(await req.json());
  } catch (error) {
  ...
  try {
    ...
    if (cards.length < 2) {
      return response({ error: '至少需要 2 张灵感卡片才能成章' }, 400);
    }

    const promptPayload = buildOutlinePromptPayload({ title: note.title, cards });
    const aiRaw = await deepseekChat(buildOutlineMessages(promptPayload), {
      model: 'deepseek-chat',
      temperature: 0.4,
    });
    const aiJson = parseOutlineAiJson(aiRaw);
    const outline = buildOutlineFromAi(
      aiJson,
      cards.map((card) => card.id),
    );
    const nextContent = { ...normalizeNoteContent(note.content), outline };
